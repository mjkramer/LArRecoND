...
"/pnfs/dune/persistent/users/noeroy/prod/MiniRun5_1E19_RHC/MiniRun5_1E19_RHC.flow.beta2a/FLOW/0000000/MiniRun5_1E19_RHC.flow.0000055.FLOW.hdf5"]
    ...
    for fname in files:
        ...
        hit_particleFileID=ROOT.std.vector("std::vector<long>")()
        ...
        mcp_file_id=ROOT.std.vector("long")()
        ...
        output_tree.Branch("hit_particleFileID",hit_particleFileID)
        ...
        output_tree.Branch("mcp_file_id",mcp_file_id)
        ...
        for ev_index in range(len(events)):
            ...
            particleFileID=ROOT.std.vector("long")()
            vertexID=ROOT.std.vector("long")()
            ...
            if (useData==False): 
                ...
                allTrajectories,allVertices,nuVertexArray,trajVertexID,trajFileID = find_all_truth_in_spill(spillArray, flow_out)
                ...
                [mcp_file_id.push_back(int(i)) for i in trajFileID]
            ...
            while hitID<len(hits_id): 
                ...
                if (useData==False):              
                    ...
                    [vertexID.push_back(int(i)) for i in contr_info[6]]
                    [particleFileID.push_back(int(i)) for i in contr_info[8]]
                ...
                hit_particleFileID.push_back(particleFileID)
                hit_vertexID.push_back(vertexID)
...
def find_tracks_in_calib_hits(hit_num, flow_out, typ="final"):
    ...
    if len(fracFromHits)<1:
        ...
        particleFile_tot.append(-999)
        vertexID_tot.append(-999)
        vertex_tot.append(-999)
    if len(segIDsFromHits)<1:
        track_contr.append(-999)
     
    return [track_contr,trackIndex_tot,segment_tot,particleIndex_tot,particle_tot,pdg_id,vertexID_tot,vertex_tot,particleFile_tot]
...
def find_all_truth_in_spill(spillID, flow_out):
    ...
    for traj_indices in traj_indicesArray:
        ...
        trajFileID.append(traj["file_traj_id"])
    ...
    return trajectories, vertices, nuVertexArray, trajVertexID, trajFileID
