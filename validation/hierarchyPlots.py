...
class parameters(object):
    ...
    def __init__(self, mcFileName, mcTreeName, evtFileName, evtTreeName):
        ...
        # List of particles we want to plot
...
def createMCHistos(pars):
    ...
    for i in range(nMC):
        ...
        mtm = getattr(mcTree, 'mcMomentum')
        nMatches = getattr(mcTree, 'nMatches')
        
        hMtmAll = hList.mtmAll
        mcMomentum = getattr(mcTree, 'mcMomentum')
        hMtmAll.Fill(mcMomentum)

        # Require at least 1 MC-reco match
        if nMatches > 0:
            ...
            purity = 0.0
            nSharedHits = 0
            for iC,comp in enumerate(completeVect):
                if comp > bestComp:
                    bestComp = comp
                    purity = purityVect[iC]
                    nSharedHits = nSharedHitsVect[iC]

            # Check for minimum completeness, purity and number of shared hits
            if bestComp >= pars.minCompleteness and purity >= pars.minPurity \
               and nSharedHits >= pars.minNSharedHits:
